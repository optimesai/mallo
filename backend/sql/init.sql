...
CREATE TABLE users (
    user_id VARCHAR(50) NOT NULL COMMENT '사내 고유 사번 (PK)',
...
CREATE TABLE item_master (
    item_code VARCHAR(50) NOT NULL COMMENT '품목 코드 (PK)',
...
CREATE TABLE partner_master (
    partner_code VARCHAR(50) NOT NULL COMMENT '거래처 고유 코드 (PK)',
...
CREATE TABLE bom_structure (
    ...
    parent_item_code VARCHAR(50) NOT NULL COMMENT '부모 품목 코드 (완제품 또는 반제품, FK)',
    child_item_code VARCHAR(50) NOT NULL COMMENT '자식 품목 코드 (반제품 또는 원자재, FK)',
    ...
    UNIQUE KEY idx_bom_tree (parent_item_code, child_item_code, bom_version)
...
CREATE TABLE warehouse_location (
    location_code VARCHAR(50) NOT NULL COMMENT '로케이션 창고 및 렉 코드 (PK, e.g., WH01-RACK02)',
...
CREATE TABLE current_inventory (
    ...
    item_code VARCHAR(50) NOT NULL COMMENT '품목 코드 (FK)',
    location_code VARCHAR(50) NOT NULL COMMENT '위치 코드 (FK)',
    ...
    FOREIGN KEY (location_code) REFERENCES warehouse_location(location_code),
    UNIQUE KEY idx_item_location (item_code, location_code)
...
CREATE TABLE inbound_receipt (
    ...
    item_code VARCHAR(50) NOT NULL COMMENT '입고 품목 코드 (FK)',
    partner_code VARCHAR(50) NOT NULL COMMENT '외주 공급사 코드 (FK)',
    location_code VARCHAR(50) NOT NULL COMMENT '적재될 창고 로케이션 코드 (FK)',
    inbound_qty INT NOT NULL COMMENT '입고 검수 수량',
    inbound_date DATE NOT NULL COMMENT '전산 입고 처리 일자',
    worker_id VARCHAR(50) COMMENT '물류 담당자 사번 (FK)',
...
CREATE TABLE inventory_transaction_history (
    ...
    item_code VARCHAR(50) NOT NULL COMMENT '품목 코드 (FK)',
    from_location VARCHAR(50) COMMENT '출발지 로케이션 코드 (FK, 입고시 NULL)',
    to_location VARCHAR(50) COMMENT '도착지 로케이션 코드 (FK, 출고시 NULL)',
    ...
    worker_id VARCHAR(50) COMMENT '처리 작업자 사번 (FK)',
...
CREATE TABLE inventory_stocktake (
    ...
    item_code VARCHAR(50) NOT NULL COMMENT '품목 코드 (FK)',
    location_code VARCHAR(50) NOT NULL COMMENT '실사 위치 코드 (FK)',
    ...
    worker_id VARCHAR(50) COMMENT '실사 담당 작업자 사번 (FK)',
...
CREATE TABLE work_order (
    order_no VARCHAR(50) NOT NULL COMMENT '생산 작업 지시 번호 (PK, e.g., WO-20260520-001)',
    item_code VARCHAR(50) NOT NULL COMMENT '생산할 완제품/반제품 품목 코드 (FK)',
...
CREATE TABLE production_execution (
    ...
    FOREIGN KEY (order_no) REFERENCES work_order(order_no),
    FOREIGN KEY (worker_id) REFERENCES users(user_id)
...
CREATE TABLE equipment_master (
    eq_code VARCHAR(50) NOT NULL COMMENT '제조 설비 고유 코드 (PK)',
...
CREATE TABLE equipment_downtime_history (
    ...
    eq_code VARCHAR(50) NOT NULL COMMENT '장애 발생 설비 코드 (FK)',
    breakdown_start TIMESTAMP NOT NULL COMMENT '고장 발생 및 라인 멈춤 시각',
    breakdown_end TIMESTAMP COMMENT '정비 완료 및 재가동 시각',
    downtime_minutes INT GENERATED ALWAYS AS (TIMESTAMPDIFF(MINUTE, breakdown_start, breakdown_end)) VIRTUAL COMMENT '라인 멈춤 다운타임 시간 자동 에산 (분)',
    ...
    worker_id VARCHAR(50) COMMENT '정비 등록 작업자 사번 (FK)',
    PRIMARY KEY (downtime_id),
    FOREIGN KEY (eq_code) REFERENCES equipment_master(eq_code),
    FOREIGN KEY (worker_id) REFERENCES users(user_id)
...
CREATE TABLE quality_inspection (
    ...
    item_code VARCHAR(50) NOT NULL COMMENT '검사 대상 품목 코드 (FK)',
    ...
    inspector_id VARCHAR(50) COMMENT '품질 담당 검사자 사번 (FK)',
...
CREATE TABLE outbound_shipping (
    shipping_no VARCHAR(50) NOT NULL COMMENT '출하 지시 번호 (PK, e.g., SH-20260520-001)',
    partner_code VARCHAR(50) NOT NULL COMMENT '납품 고객사 코드 (FK)',
    item_code VARCHAR(50) NOT NULL COMMENT '출하할 완제품 품목 코드 (FK)',
    request_qty INT NOT NULL COMMENT '고객사 주문 확정 수량',
    picking_location VARCHAR(50) COMMENT '완제품 창고 피킹 적재 위치 코드 (FK)',
    ...
    FOREIGN KEY (partner_code) REFERENCES partner_master(partner_code),
    FOREIGN KEY (item_code) REFERENCES item_master(item_code),
    FOREIGN KEY (picking_location) REFERENCES warehouse_location(location_code),
    FOREIGN KEY (worker_id) REFERENCES users(user_id)
...
CREATE TABLE ai_query_history (
    ...
    worker_id VARCHAR(50) NOT NULL COMMENT '자연어 질문을 던진 유저 사번 (FK)',
...
CREATE TABLE dynamic_batch_schedule (
    ...
    worker_id VARCHAR(50) COMMENT '배치 동적 등록 및 관리자 사번 (FK)',
