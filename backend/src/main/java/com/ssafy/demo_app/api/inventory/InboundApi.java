...
@Tag(name = "Inbound Management API", description = "원부자재 입고 관리 API")
@RequestMapping("/api/v2/inbounds")
public interface InboundApi {
    ...
    ResponseEntity<ApiResponse<PageResponse<InboundReceiptResponse>>> getInbounds(
            ...
            @Parameter(description = "입고 상태 (READY, COMPLETED, STACKED)")
    ...
    @Operation(summary = "입고 완료 처리", description = "READY 상태의 입고를 검수하여 COMPLETED 처리합니다.")
    @PutMapping("/{id}/complete")
    ResponseEntity<ApiResponse<InboundReceiptResponse>> completeInbound(
            @Parameter(description = "입고 ID", required = true) @PathVariable Integer id
    );

    @Operation(summary = "재고 적재 및 로케이션 지정", description = "입고 완료된 자재를 로케이션에 적재하고 STACKED 처리합니다.")
