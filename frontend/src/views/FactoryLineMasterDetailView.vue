...
<template>
  <div class="space-y-6 pb-12">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <button class="mb-4 inline-flex items-center gap-2 rounded-xl app-bg-muted px-4 py-2 app-type-sm app-font-emphasis app-text-soft" type="button" @click="router.push({ name: 'factory-line-master' })">
          <ArrowLeft class="h-4 w-4" />
          목록으로
        </button>
        <h1 class="flex items-center gap-2 app-type-2xl app-font-emphasis tracking-tight app-text-strong">
          <Factory class="h-6 w-6" />
          공장 및 생산라인 상세
        </h1>
        <p class="mt-1 app-type-sm app-font-label app-text-muted">선택한 공장/라인/공정 라우팅 기준정보와 참조 현황을 확인합니다.</p>
      </div>
      <div v-if="successToast" class="rounded-xl border app-border app-bg-success-soft px-4 py-2 app-type-sm app-font-strong app-text-success">
        {{ successToast }}
      </div>
    </div>
    ...
    <template v-else-if="routing">
      <section class="rounded-3xl border app-border app-bg-surface p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p class="app-type-xs app-font-emphasis uppercase tracking-widest app-text-muted">ROUTING-ID {{ routing.routingId }}</p>
            <h2 class="mt-2 app-type-2xl app-font-emphasis app-text-strong">{{ routing.factoryName }} / {{ routing.lineName }}</h2>
            <p class="mt-1 app-font-strong app-text-soft">{{ routing.operationSeq }}. {{ routing.operationName }}</p>
          </div>
          <div class="flex flex-wrap gap-2">
            <span class="rounded-full px-3 py-2 app-type-sm app-font-emphasis" :class="routing.routingStatus === 'ACTIVE' ? 'app-status-success' : 'app-status-warning'">
              {{ getRoutingStatusLabel(routing.routingStatus) }}
            </span>
            <button v-if="canManageMasterData" class="rounded-xl app-bg-strong px-4 py-2 app-type-sm app-font-emphasis app-text-inverse" type="button" @click="openEditForm">
              <Edit3 class="mr-1 inline h-4 w-4" />
              수정
            </button>
            <button v-if="canManageMasterData" class="rounded-xl app-bg-muted px-4 py-2 app-type-sm app-font-emphasis app-text-soft" type="button" @click="toggleRoutingStatus">
              {{ routing.routingStatus === 'ACTIVE' ? '비활성화' : '활성화' }}
            </button>
            <button v-if="canDeleteRouting" class="rounded-xl app-bg-danger px-4 py-2 app-type-sm app-font-emphasis app-text-inverse" type="button" @click="requestDelete">
      ...
      <section class="rounded-3xl border app-border app-bg-surface shadow-sm">
        <div class="flex flex-wrap gap-2 border-b app-border-muted p-4">
          <button class="rounded-xl px-4 py-2 app-type-sm app-font-emphasis" :class="activeDetailTab === 'operation' ? 'app-bg-strong app-text-inverse' : 'app-bg-muted app-text-soft'" type="button" @click="activeDetailTab = 'operation'">공정 기준</button>
          <button class="rounded-xl px-4 py-2 app-type-sm app-font-emphasis" :class="activeDetailTab === 'line' ? 'app-bg-strong app-text-inverse' : 'app-bg-muted app-text-soft'" type="button" @click="activeDetailTab = 'line'">라인 상세</button>
          <button class="rounded-xl px-4 py-2 app-type-sm app-font-emphasis" :class="activeDetailTab === 'flow' ? 'app-bg-strong app-text-inverse' : 'app-bg-muted app-text-soft'" type="button" @click="activeDetailTab = 'flow'">공정 플로우</button>
          <button class="rounded-xl px-4 py-2 app-type-sm app-font-emphasis" :class="activeDetailTab === 'system' ? 'app-bg-strong app-text-inverse' : 'app-bg-muted app-text-soft'" type="button" @click="activeDetailTab = 'system'">시스템 정보</button>
        </div>
        ...
        <div class="p-6">
          <div v-if="activeDetailTab === 'operation'" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-2xl app-bg-muted p-5">
              ...
              <dl class="grid grid-cols-[8rem_1fr] gap-3 app-type-sm">
          ...
          <div v-else-if="activeDetailTab === 'flow'" class="flex flex-wrap items-center gap-3">
            <template v-for="(operation, index) in lineOperations" :key="operation.routingId">
              ...
              >
                <span class="block app-type-xs app-font-emphasis app-text-muted">{{ operation.operationSeq }} 공정</span>
                <strong class="mt-1 block app-font-emphasis">{{ operation.operationName }}</strong>
                <span class="mt-1 block app-type-xs">ROUTING-ID {{ operation.routingId }}</span>
              </button>
          ...
          <div v-else class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div class="rounded-2xl app-bg-warning-soft p-5">
              ...
              <p class="app-type-sm app-font-strong app-text-warning">{{ usage?.recommendedAction || '참조 현황을 불러오고 있습니다.' }}</p>
              <p v-if="usage?.workOrderNos.length" class="mt-3 app-type-sm app-font-strong app-text-soft">작업지시: {{ usage.workOrderNos.join(', ') }}</p>
              <p v-if="usage?.executionIds.length" class="mt-2 app-type-sm app-font-strong app-text-soft">실적 ID: {{ usage.executionIds.join(', ') }}</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <div v-if="isFormOpen" class="fixed inset-0 z-50 flex items-center justify-center app-backdrop p-4">
      <div class="w-full max-w-2xl rounded-3xl app-bg-surface shadow-2xl">
        <div class="flex items-center justify-between border-b app-border-muted p-6">
          <h2 class="app-type-lg app-font-emphasis app-text-strong">공장/생산라인 라우팅 수정</h2>
        ...
        <form class="space-y-4 p-6" @submit.prevent="submitForm">
          ...
          <label class="block app-type-sm app-font-emphasis app-text-soft">
            공장명 *
            <input v-model="form.factoryName" maxlength="50" required class="mt-1 h-11 w-full rounded-2xl border app-border px-4 app-type-sm outline-none">
          </label>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label class="block app-type-sm app-font-emphasis app-text-soft">
              생산 라인명 *
              <input v-model="form.lineName" maxlength="50" required class="mt-1 h-11 w-full rounded-2xl border app-border px-4 app-type-sm outline-none">
            </label>
            <label class="block app-type-sm app-font-emphasis app-text-soft">
              공정 순서 *
              <input v-model.number="form.operationSeq" min="1" required type="number" class="mt-1 h-11 w-full rounded-2xl border app-border px-4 app-type-sm outline-none">
            </label>
          </div>
          <label class="block app-type-sm app-font-emphasis app-text-soft">
            세부 공정명 *
            <input v-model="form.operationName" maxlength="50" required class="mt-1 h-11 w-full rounded-2xl border app-border px-4 app-type-sm outline-none">
          </label>
          <div class="rounded-2xl app-bg-muted p-3 app-type-sm app-font-strong app-text-muted">
          ...
          <div class="flex justify-end gap-2 border-t app-border-muted pt-4">
            ...
            <button class="rounded-2xl app-bg-strong px-5 py-2.5 app-type-sm app-font-emphasis app-text-inverse" type="submit" :disabled="factoryRoutingStore.isSaving">수정 저장</button>
          </div>
        </form>
      </div>
    </div>
