...
<template>
  <section class="min-h-full bg-slate-100 p-6 text-slate-900">
    ...
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <button v-if="isAdmin" class="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-slate-300 transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60" type="button" :disabled="userStore.isLoading" @click="fetchUsers">
    ...
    <template v-else>
      <div class="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex items-center justify-between text-sm font-bold text-slate-500">전체 사용자 <Users class="h-5 w-5 text-slate-400" /></div>
      ...
      <p v-if="pageError" class="mb-4 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700">{{ pageError }}</p>
      ...
      <div v-if="pageMode === 'list'" class="space-y-5">
        <form class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" @submit.prevent="submitSearch">
          <div class="grid gap-3 lg:grid-cols-[1fr_260px_auto_auto]">
            <label class="relative block">
              ...
              <input v-model="keywordInput" class="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-semibold outline-none transition focus:border-slate-500 focus:bg-white" placeholder="사번, 이름, 부서 검색" @focus="isSuggestOpen = true" @input="isSuggestOpen = true">
              <div v-if="isSuggestOpen && keywordSuggestions.length > 0" class="absolute left-0 right-0 top-14 z-30 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                <button v-for="candidate in keywordSuggestions" :key="`${candidate.label}-${candidate.user.userId}`" class="flex w-full items-center justify-between gap-4 px-4 py-3 text-left transition hover:bg-slate-50" type="button" @mousedown.prevent="selectSuggestion(candidate.user)">
                  <span>
                  ...
                  <span class="rounded-full border px-2 py-1 text-[11px] font-black" :class="getRoleBadgeClass(candidate.user.role)">{{ getRoleLabel(candidate.user.role) }}</span>
                </button>
              </div>
            </label>
            <select v-model="roleFilter" class="h-12 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold outline-none transition focus:border-slate-500 focus:bg-white">
            ...
            <button class="h-12 rounded-2xl bg-slate-950 px-6 text-sm font-black text-white transition hover:bg-slate-800" type="submit">검색</button>
            <button class="h-12 rounded-2xl border border-slate-200 px-5 text-sm font-bold text-slate-600 transition hover:bg-slate-100" type="button" @click="resetFilters">초기화</button>
          </div>
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div v-if="userStore.isLoading" class="flex h-72 items-center justify-center gap-3 text-sm font-bold text-slate-500"><Loader2 class="h-5 w-5 animate-spin" />사용자 정보를 불러오는 중입니다.</div>
          <div v-else class="overflow-x-auto">
            <table class="w-full min-w-[860px] text-left text-sm">
              <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-400">
                <tr><th class="px-5 py-3 font-black">사번</th><th class="px-5 py-3 font-black">이름</th><th class="px-5 py-3 font-black">부서</th><th class="px-5 py-3 font-black">권한</th><th class="px-5 py-3 font-black">가입일</th></tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                <tr v-for="targetUser in filteredUsers" :key="targetUser.userId" class="cursor-pointer transition hover:bg-slate-50" @click="openUserDetail(targetUser)">
                  <td class="px-5 py-4 font-black text-slate-900">{{ targetUser.employeeNo }}</td>
                  <td class="px-5 py-4 font-semibold text-slate-700">{{ targetUser.userName }}</td>
                  <td class="px-5 py-4 text-slate-500">{{ targetUser.department }}</td>
                  <td class="px-5 py-4"><span class="inline-flex rounded-full border px-2.5 py-1 text-xs font-black" :class="getRoleBadgeClass(targetUser.role)">{{ getRoleLabel(targetUser.role) }}</span></td>
                  <td class="px-5 py-4 text-xs font-semibold text-slate-400">{{ formatDateTime(targetUser.createdAt) }}</td>
                </tr>
              </tbody>
            </table>
            <div v-if="filteredUsers.length === 0" class="px-5 py-16 text-center text-sm font-bold text-slate-400">조건에 맞는 사용자가 없습니다.</div>
          </div>
        </div>
      </div>

      <div v-else class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <button class="mb-5 inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-600 transition hover:bg-slate-50" type="button" @click="pageMode = 'list'"><ArrowLeft class="h-4 w-4" />목록으로</button>
        <div v-if="selectedUser" class="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
          <div class="rounded-3xl bg-slate-950 p-6 text-white">
            <div class="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10"><UserCog class="h-7 w-7" /></div>
            ...
            <span class="mt-5 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-black">{{ getRoleLabel(selectedUser.role) }}</span>
          </div>
          <div class="space-y-5">
            <div class="grid gap-3 md:grid-cols-3">
              <div class="rounded-2xl border border-slate-100 p-4"><div class="text-xs font-black text-slate-400">권한 설명</div><div class="mt-2 text-sm font-semibold text-slate-700">{{ getRoleDescription(selectedUser.role) }}</div></div>
              <div class="rounded-2xl border border-slate-100 p-4"><div class="text-xs font-black text-slate-400">가입일</div><div class="mt-2 text-sm font-semibold text-slate-700">{{ formatDateTime(selectedUser.createdAt) }}</div></div>
              <div class="rounded-2xl border border-slate-100 p-4"><div class="text-xs font-black text-slate-400">수정일</div><div class="mt-2 text-sm font-semibold text-slate-700">{{ formatDateTime(selectedUser.updatedAt) }}</div></div>
            </div>
            <div class="rounded-2xl border border-slate-200 p-5">
              <label class="text-xs font-black text-slate-400" for="role-select">권한 변경</label>
              <select id="role-select" v-model="pendingRole" class="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-bold outline-none transition focus:border-slate-500 focus:bg-white disabled:cursor-not-allowed disabled:opacity-60" :disabled="!canChangeSelectedRole">
              ...
              <button class="mt-3 flex h-12 w-full items-center justify-center rounded-xl bg-slate-950 text-sm font-black text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50" type="button" :disabled="!canChangeSelectedRole || pendingRole === selectedUser.role" @click="applyRoleChange">권한 저장</button>
            </div>
            <div class="rounded-2xl border border-rose-100 bg-rose-50 p-5">
              ...
              <button class="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-rose-600 text-sm font-black text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-50" type="button" :disabled="!canDeleteSelectedUser" @click="requestDelete(selectedUser)"><Trash2 class="h-4 w-4" />사용자 삭제</button>
