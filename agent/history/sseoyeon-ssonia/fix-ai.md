...
### 라인 차트 가독성 개선 (Codex)
- **User Intent**: AI 챗봇 라인 그래프가 축 지표와 포인트 값 없이 선만 표시되어 날짜별 수량을 분간하기 어려우므로, x축/y축 지표와 각 포인트 값을 추가 요청
- **Agent Context**: 기존 `AiChartPanel`의 LINE 렌더링은 SVG path와 하단 라벨만 표시하고 y축 눈금, 축명, 포인트 값, 포인트 마커가 없어 추이 해석이 어려웠다. 기존 차트 응답 계약은 유지하고 프론트 렌더링만 보강.
- **Key Decisions**:
  - 백엔드 응답 구조는 변경하지 않음 — 이미 `xKey`, `yKeys`가 있으므로 프론트에서 축명과 포인트 값을 계산해 표시
  - 라인 차트 좌표 계산을 명시적 차트 영역으로 분리 — 축/눈금/포인트/라벨이 같은 좌표계를 공유하도록 구성
  - 패널과 모달에 동일한 축 정보를 표시 — 작은 카드와 크게 보기 모두에서 값 확인이 가능하도록 중복 렌더링을 일관되게 갱신
- **Affected Files**: <details><summary>1개 파일</summary>

  - **Created**:
    - 없음
  - **Modified**:
    - `frontend/src/ui/AiChartPanel.vue` (+170/-18) — LINE 차트 x축/y축, y축 눈금, 포인트 마커, 포인트 값, 축 라벨 표시 추가
  - **Deleted**:
    - 없음
