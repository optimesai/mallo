...
- 히스토리 로그는 태스크 종료 시점에 1회 기록하며, 컴포넌트 커밋마다 기록하지 않는다.
...
- 백엔드 코드 변경 시 `./gradlew compileJava`로 컴파일 오류를 확인한다.
- 프론트엔드 코드 변경 시 `npm run build` 또는 `vue-tsc --noEmit`으로 타입 검사를 확인한다.
- 빌드 실패 상태로 커밋해서는 안 된다.
