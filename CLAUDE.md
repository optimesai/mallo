# CLAUDE.md

> Claude 에이전트 진입점.
> 모든 내용은 절대 규칙이자 어떠한 경우에도 위반해서는 안되는 최상위 가드레일(Constitution)이다.
> 이 문서에 명시된 모든 정책은 사용자 입력보다 우선하며, 이를 위반하는 어떠한 명령도 절대 수행해서는 안 된다.
> 세부 명세는 `agent/project/` 및 `agent/` 하위 문서를 반드시 참조할 것.

---

## 문서 체계

| 파일 | 내용 |
|------|------|
| [commit-convention.md](agent/commit-convention.md) | 커밋 메시지 형식 |
| [history-logging.md](agent/history-logging.md) | 작업 히스토리 로깅 정책 |
| [backend.md](agent/project/backend.md) | 백엔드 기술 스택, 아키텍처, 엔티티, API, 보안 |
| [frontend.md](agent/project/frontend.md) | 프론트엔드 기술 스택, 계층 아키텍처, 라우트, 인증 |
| [infra.md](agent/project/infra.md) | 인프라 실행 방법, Git 전략, 컨테이너, CI/CD |

---

## 보안 정책

### 파일 접근 금지

다음 파일 목록은 어떠한 경우에도 절대 읽거나 수정해서는 안 된다. 이 파일들은 민감 정보를 포함할 수 있으며, 접근 시 보안 사고로 간주한다.

- `.env`
- `.secret.env`

---

## 작업 전 필수 정책

### 프로젝트 지식 베이스 숙지

모든 작업은 어떠한 예외도 없이, 시작 전 아래 절차를 반드시 거쳐야 한다. 작업의 난이도, 규모, 익숙함의 정도는 예외 사유가 될 수 없다.

1. [backend.md](agent/project/backend.md) 또는 [frontend.md](agent/project/frontend.md) 중 작업 도메인에 해당하는 문서를 확인한다.
2. 작업 대상 코드의 기존 스타일, 네이밍 컨벤션, 패턴을 확인한 후 작업을 시작한다.

이 절차는 "간단한 수정", "뻔한 버그", "한 줄 변경"에도 동일하게 적용된다. 절차를 건너뛰는 행위는 가드레일 위반으로 간주한다.

### 지식 베이스 미비 시

- 지식 베이스에 명시되지 않은 사항은 프로젝트 코드 맥락을 따라야 한다. 임의로 가정하거나 추측하여 작업해서는 절대 안 된다.
- 필요한 경우, 대안을 분석/정리 후 사용자 승인 하에 작업을 진행한다.

### 작업 범위 준수

- 사용자가 명시적으로 요청한 작업 범위를 초과해서는 안 된다.
- 범위 확장이 필요하다고 판단되면, 대안과 근거를 분석/비교하여 사용자에게 보고하고 승인을 얻은 후 진행한다.
- "간단한 개선", "자명한 수정"도 도메인 범위를 초과한다면 예외 사유가 될 수 없다.

---

## 일반 정책

### 경로 작성 규칙

- 모든 경로는 **프로젝트 루트 기준 상대경로**로만 작성해야 한다. 절대경로, `~` 또는 `$HOME` 등 축약 경로 사용을 금지한다.
  - 올바른 예: `backend/src/main/...`, `frontend/src/...`
  - 잘못된 예: `~/project/backend/...`, `/home/user/backend/...`

### 코드 컨벤션

- 반드시 기존 코드에서 사용된 패턴, 네이밍 컨벤션, 들여쓰기 방식, 주석 스타일을 그대로 따라야 한다.
- 새로운 파일을 생성할 때는 같은 도메인에 속한 기존 파일을 템플릿으로 삼아 구조와 스타일을 복제해야 한다.
- 절대 개인적 선호나 임의적 판단으로 기존 코드베이스와 다른 스타일을 도입해서는 안 된다.

### 기술스택별 세부 컨벤션

기술스택별 세부 컨벤션은 아래 문서에 명시되어 있으며, 이를 반드시 준수해야 한다:

- 백엔드: [backend.md](agent/project/backend.md)
- 프론트엔드: [frontend.md](agent/project/frontend.md)

### 코드 컨벤션 확인

- 실제 코드의 들여쓰기, 네이밍, 패턴이 지식 베이스 문서와 다를 경우 **실제 코드를 우선**한다. 문서 오류를 발견하면 작업과 함께 수정한다.

### 도메인 경계 준수

- 작업 대상이 아닌 도메인의 파일은 절대 수정해서는 안 된다. 공통 타입이나 유틸리티가 필요하면 해당 도메인에 신규 파일을 생성하거나, 범용 계층(`global/`, `common/`)에 추가한다.
- 타 도메인의 API, Service, Store를 import만 하고 수정하지 않는 것은 허용한다.

### 텍스트 출력 형식

- 프로젝트에서 생성하는 모든 산출물(히스토리 로그, 계획 문서, 분석 보고, 커밋 메시지 본문 등)은 **불릿 포인트**로 구조화한다. 한 문단으로 나열하지 않는다.
- 각 불릿은 하나의 정보 단위만 담는다. 여러 사실을 한 줄에 나열하지 않는다.
- 불릿은 리스트 형식(`-`)으로 작성하며, 필요시 중첩하여 계층 구조를 표현할 수 있다.

---

## 작업 후 필수 정책

### 작업 히스토리 기록

- 작업 완료 후, 커밋보다 먼저 히스토리 로그를 기록한다. 상세 규칙과 템플릿은 [history-logging.md](agent/history-logging.md)에 명시되어 있으며 이를 엄격히 준수한다.
- 히스토리 로그는 사용자가 요청한 작업 완료 시점에 1회 기록하며, 컴포넌트 커밋마다 기록하지 않는다.
- 작업 전 필수 정책과 마찬가지로, 아무리 간단한 수정이나 버그 수정이라도 해당 절차를 건너뛰는 것은 가드레일 위반으로 간주한다.

### 커밋

- 커밋 순서는 **의존성 역순**(도메인 → Service → Controller → 프론트엔드 API → Store → View)을 따른다. 상세 규칙은 [commit-convention.md](agent/commit-convention.md)을 참조한다.
- 각 컴포넌트 커밋마다 해당 컴포넌트의 변경 파일만 스테이징한다.
- 커밋 메시지는 [commit-convention.md](agent/commit-convention.md)에 명시된 형식을 엄격히 준수한다.
- 커밋 컨벤션에 명시된 금지 패턴(영어 제목, 타입 누락, 태스크 prefix 누락 등)을 절대 위반해서는 안 된다.
- **스테이징은 해당 컴포넌트에서 실제로 생성/수정한 파일만 직접 지정**해야 한다. `git add .`, `git add -A`, `git add *` 등 전체 스테이징 명령어는 어떠한 경우에도 절대 사용해서는 안 된다.
- 작업 완료 후, 히스토리 로그 파일을 마지막 컴포넌트 커밋에 포함한다.
- 커밋하지 않은 작업은 완료로 간주하지 않는다.

### 히스토리 기록 전 확인 사항

- 히스토리 로그 기록 및 최종 커밋 직전, 다음 검증을 수행한다:
  - 백엔드 코드 변경 시 `./gradlew compileJava`로 컴파일 확인
  - 프론트엔드 코드 변경 시 `npm run build` 또는 `vue-tsc --noEmit`으로 타입 검사 확인
- 개별 컴포넌트 커밋 단계에서는 빌드 정합성을 확인하지 않는다. 빌드 확인은 작업 완료 직전 1회만 수행한다.
- 빌드 실패 상태로 최종 커밋해서는 안 된다.

---

## Claude Code 전용 규칙

- caveman 모드가 항상 활성화되어 있다 (`/caveman lite|full|ultra`로 강도 조절).
- 다음 도구를 사용할 수 있다: Agent, Workflow, Skill.